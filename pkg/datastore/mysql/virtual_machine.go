...
func (m *MySQL) GetVirtualMachine(vmID uuid.UUID) (*ganymede.VirtualMachine, error) {
	...
	query := fmt.Sprintf(`SELECT uuid, name, vcpus, memory_kib, hypervisor_name, root_volume_id, volume.capacity_gb, read_bytes_sec, write_bytes_sec, read_iops_sec, write_iops_sec, volume.base_image_id, cpu_pinning_group_id FROM virtual_machine JOIN volume ON virtual_machine.root_volume_id = volume.id WHERE virtual_machine.uuid = '%s'`, vmID.String())
...
func (m *MySQL) PutVirtualMachine(vm ganymede.VirtualMachine) error {
	query := `INSERT INTO virtual_machine(name, uuid, vcpus, memory_kib, hypervisor_name, root_volume_id, read_bytes_sec, write_bytes_sec, read_iops_sec, write_iops_sec, cpu_pinning_group_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	_, err := m.Conn.Exec(query, vm.Name, vm.UUID, vm.Vcpus, vm.MemoryKiB, vm.HypervisorName, vm.RootVolumeID, vm.ReadBytesSec, vm.WriteBytesSec, vm.ReadIOPSSec, vm.WriteIOPSSec, vm.CPUPinningGroupID)
	if err != nil {
